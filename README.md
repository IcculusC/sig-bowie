...
First you will need a Spotify Web API client ID and client secret.  The Spotify Web API docs and resources are located [here](https://beta.developer.spotify.com/documentation/web-api/).

In the root of the repository, create a file named `.env.development.local` with the following content.
...
Next, run `yarn install` or `npm install`.

Finally, run `yarn start` or `npm run start`.

If the launcher doesn't open your browser automatically, navigate to [localhost:3000/](localhost:3000/).

## Features

- React/Redux application with `redux-observable` for asynchronous actions
- `Rx.DOM.ajax` for `Observable` ajax responses in `redux-observable` epics
- Store persistence; any albums loaded will not need to be refetched on subsequent page loads, but I did not support clearing the local cache from the UI.  The cache can be cleared from dev console by removing the `persist:root` key from local storage.
- `material-ui@next` for UI components, `jss` and flexbox to style them
- Infinite scrolling on bump scroll up to 50 albums; deduplicates any repeated responses, but there are multiple versions of many results.
- Responsive to ~328px
- Built with `webpack`, project generated with `create-react-app` and ejected for customization.
- 90%+ test coverage with `yarn test -- --coverage` or `npm run test -- --coverage`
- Supports the Redux dev tools extension in `development` environment, middleware is excluded in `production`.

## Issues

The Spotify Web API is no longer open, so I used an npm package called `spotify-web-api-node` to handle client credentials.  A limitation of this new API is that bearer tokens from client credentials only last for one hour, so it's possible that the simple backend in the `config/webpackDevServer.config.js` could fail when the token expires.  I did add handling for this case, but it's not thoroughly tested.
&nbsp;
...
&nbsp;
## Everything Below Generated By `create-react-app`
